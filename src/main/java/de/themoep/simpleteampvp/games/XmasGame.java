...
public class XmasGame extends SimpleTeamPvPGame {
    ...
    public void onItemDrop(PlayerDropItemEvent event) {
        ...
        if (!event.getItemDrop().getItemStack().isSimilar(getConfig().getPointItem())) {
            event.setCancelled(true);
        }
    }
    ...
    public void checkForPointItemOnPlayerDeath(PlayerDeathEvent event) {
        ...
        if (event.getKeepInventory()) {
            ...
            while (dropIterator.hasNext()) {
                ...
                if (!drop.isSimilar(getConfig().getPointItem())) {
                    dropIterator.remove();
                }
            }
        }
    ...
    public void onPointBlockBreak(BlockBreakEvent event) {
        ...
        if (event.getBlock().getType() == getConfig().getPointBlock() ||
                (getConfig().getPointBlock() == Material.GLOWING_REDSTONE_ORE && event.getBlock().getType() == Material.REDSTONE_ORE) ||
                (getConfig().getPointBlock() == Material.REDSTONE_ORE && event.getBlock().getType() == Material.GLOWING_REDSTONE_ORE)
                ) {
            event.setCancelled(true);
            event.getBlock().setType(Material.AIR);
            addToPointBlockSet(event.getBlock().getLocation());
            if (getConfig().getPointItem() != null) {
                event.getPlayer().getInventory().addItem(getConfig().getPointItem());
            } else {
